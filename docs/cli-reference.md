...
| 6 | 품질 점수 평가 | 선택 | Teacher LLM이 QA 쌍을 1~5점으로 평가합니다. 저품질 QA 재생성 옵션을 포함합니다. 건너뛸 수 있습니다. |
...
slm-factory의 파싱·QA 데이터를 AutoRAG 평가용 parquet 형식으로 내보냅니다. `parsed_documents.json`에서 문서 청크를 생성하고, QA 파일에서 평가 데이터를 추출하여 AutoRAG가 요구하는 `corpus.parquet`과 `qa.parquet`을 생성합니다.
