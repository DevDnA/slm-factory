...
# 특정 단계까지 재개하며 실행
slm-factory run --until augment --resume
...
# 기존 학습 데이터로 학습만 실행
slm-factory train --data ./my-project/output/training_data.jsonl

# 외부에서 준비한 데이터로 학습
slm-factory train --data ./custom_data.jsonl
...
# 특정 어댑터 경로 지정
slm-factory export --adapter ./my-project/output/checkpoints/adapter
```
...
# 특정 QA 데이터로 평가
slm-factory eval run --model my-project-model --data ./my-project/output/qa_alpaca.json
```
...
slm-factory tool review

# 특정 파일로 리뷰 시작
slm-factory tool review --data ./my-project/output/qa_alpaca.json
```
...
```bash
# 자동 감지된 QA 데이터로 변환
slm-factory tool convert

# 특정 파일로 변환
slm-factory tool convert --data ./my-project/output/qa_augmented.json
```
...
slm-factory tool evolve

# 모든 문서 재처리 (변경 감지 건너뜀)
slm-factory tool evolve --force-update

# 품질 게이트 건너뜀 (무조건 배포)
slm-factory tool evolve --skip-gate

# 모든 옵션 적용
slm-factory tool evolve --force-update --skip-gate
```
...
```
slm-factory tool compare-data [OPTIONS]
```
...
```
slm-factory tool export-autorag [OPTIONS]
```
...
# 코퍼스 디렉토리 직접 지정
slm-factory tool rag-index --corpus-dir ./output/autorag
```
