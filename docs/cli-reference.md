...
| # | 단계 | 유형 | 설명 |
|---|------|:----:|------|
| 1 | 문서 파싱 | 필수 | 선택한 문서를 자동으로 파싱합니다 |
| 1b | 문서 청킹 | 선택 | 긴 문서를 청크로 분할하여 QA 생성 범위를 확장합니다 (`chunking.enabled` 설정 반영) |
| 2 | QA 쌍 생성 | 필수 | Teacher LLM으로 질문-답변 쌍을 생성합니다. 확인 후 진행합니다. |
| 3 | QA 검증 | 필수 | 규칙 기반 및 임베딩 기반 검증을 자동으로 실행합니다 |
| 4 | 품질 점수 평가 | 선택 | Teacher LLM이 QA 쌍을 1~5점으로 평가합니다. 저품질 QA 재생성 옵션을 포함합니다. 건너뛸 수 있습니다. (`scoring.enabled` 설정 반영) |
| 5 | 데이터 증강 | 선택 | 질문을 다양한 표현으로 변형하여 데이터를 늘립니다. 건너뛸 수 있습니다. (`augment.enabled` 설정 반영) |
