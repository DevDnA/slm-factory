...
```
uv run slm-factory [전역 옵션] <명령어> [옵션]
```
...
```
uv run slm-factory init <name> [OPTIONS]
```
...
# 특정 경로에 생성
uv run slm-factory init policy-assistant --path /workspace/projects
```
...
```
uv run slm-factory check [OPTIONS]
```
...
```
uv run slm-factory run [OPTIONS]
```
...
```
uv run slm-factory train [OPTIONS]
```
...
```
uv run slm-factory export [OPTIONS]
```
...
```
uv run slm-factory eval run [OPTIONS]
```
...
```
uv run slm-factory eval compare [OPTIONS]
```
...
```
uv run slm-factory tool wizard [OPTIONS]
```
...
| 6 | 품질 점수 평가 | 선택 | Teacher LLM이 QA 쌍을 1~5점으로 평가합니다. 저품질 QA 재생성 옵션을 포함합니다. 건너뛸 수 있습니다. (`scoring.enabled` 설정 반영) |
| 7 | 데이터 증강 | 선택 | 질문을 다양한 표현으로 변형하여 데이터를 늘립니다. 건너뛸 수 있습니다. (`augment.enabled` 설정 반영) |
| 8 | 통계 분석 | 자동 | 카테고리별 QA 분포, 길이 통계 등 분석 보고서를 생성합니다 (`analyzer.enabled` 설정 반영) |
| 9 | 데이터 변환 | **자동** | QA 데이터를 채팅 템플릿 적용 JSONL 형식으로 변환합니다 |
| 10 | 모델 학습 | 필수 | Student 모델에 LoRA 어댑터를 적용하여 파인튜닝합니다. 확인 후 진행합니다. |
| 11 | 모델 내보내기 | 필수 | LoRA 어댑터를 병합하고 Ollama Modelfile을 생성합니다. 확인 후 진행합니다. |
| 12 | 멀티턴 대화 생성 | 선택 | QA 쌍을 멀티턴 대화 형식으로 확장합니다. 건너뛸 수 있습니다. |
| 13 | GGUF 변환 | 선택 | 모델을 llama.cpp 호환 GGUF 형식으로 변환합니다. 건너뛸 수 있습니다. (`gguf_export.enabled` 설정 반영) |
| 14 | 모델 평가 | 선택 | BLEU/ROUGE 메트릭으로 학습된 모델을 평가합니다. 건너뛸 수 있습니다. |
| 15 | 코퍼스 내보내기 | 선택 | QA·문서를 RAG 인덱싱용 parquet으로 내보냅니다. 건너뛸 수 있습니다. (`autorag_export.enabled` 설정 반영) |
| 16 | RAG 인덱싱 | 선택 | corpus.parquet을 ChromaDB에 임베딩하여 적재합니다. 건너뛸 수 있습니다. |
...
```
uv run slm-factory tool review [OPTIONS]
```
...
```
uv run slm-factory tool dashboard [OPTIONS]
```
...
```
uv run slm-factory tool convert [OPTIONS]
```
...
```
uv run slm-factory tool evolve [OPTIONS]
```
...
```
uv run slm-factory tool compare-data [OPTIONS]
```
...
```bash
# 청킹 전후 QA 품질 비교
uv run slm-factory tool compare-data \
...
```
uv run slm-factory tool export-autorag [OPTIONS]
```
...
```
uv run slm-factory tool rag-index [OPTIONS]
```
...
```
uv run slm-factory tool rag-serve [OPTIONS]
```
...
```
uv run slm-factory tool update [OPTIONS]
```
...
```
uv run slm-factory status [OPTIONS]
```
...
```
uv run slm-factory clean [OPTIONS]
```
