...
전체 파이프라인은 독립적인 단계(parse, chunking, ontology, generate, validate, score, augment, analyze, convert, train, export)로 구성됩니다. 각 단계는 CLI 명령어로 개별 실행 가능하며, 이전 단계의 출력 파일을 입력으로 사용합니다. 특정 단계만 재실행하거나 중간 결과를 검토할 수 있습니다.
...
┌──────────────────┐ ┌──────────────────┐ ┌──────────────────┐ ┌──────────────────┐
  1a. Ontology (선택)  3a. Score            3b. Augment          3c. Analyze
  ontology/            scorer.py            augmenter.py         analyzer.py
└────────┬─────────┘ └────────┬─────────┘ └────────┬─────────┘ └────────┬─────────┘
         ▼                    ▼                    ▼                    ▼
  ontology.json        qa_scored.json       qa_augmented.json   data_analysis.json
...
cli.py
  └─→ pipeline.py
        ...
        ├─→ ontology/
        │     ├─→ models.py (Entity, Relation, KnowledgeGraph)
        │     ├─→ extractor.py (OntologyExtractor)
...
### 3.5 비동기 동시성 패턴 (scorer, augmenter, ontology extractor, regeneration)

Scorer, Augmenter, OntologyExtractor, 그리고 저품질 QA 재생성 루프는 `asyncio.Semaphore`로 동시 요청 수를 제한하면서 병렬 처리합니다.
...
| ontology (선택) | `list[ParsedDocument]` | `KnowledgeGraph` | `ontology.json` |
...
│   ├── enabled: bool = False
│   ├── entity_types: list[str] = ["Person", "Organization", ...]
│   ├── max_concurrency: int = 4
...
wizard
  ...
  │    └─ 자동 진행 (확인 없음)
  │
  ├─ Step 3a. 온톨로지 (선택적) ──── Confirm(default=ontology.enabled) → step_extract_ontology()
  │    └─ config.ontology.enabled 기본값 반영
  │
...
| **선택 (설정 의존)** | 온톨로지 추출, 품질 평가, 데이터 증강, 멀티턴 대화, GGUF 변환, 모델 평가 | `Confirm.ask(default=config값)` | config 의존 |
