...
┌──────────────────┐ ┌──────────────────┐ ┌──────────────────┐ ┌──────────────────┐ ┌──────────────────┐
  10. Dialogue         11. GGUF Export      12. Eval             13. Autorag Export   14. RAG Index
...
wizard
  ...
  │
  ├─ Step 13. 코퍼스 내보내기 (선택적) ── Confirm(default=autorag_export.enabled) → step_autorag_export()
...
| **선택 (설정 의존)** | 품질 평가, 데이터 증강, 멀티턴 대화, GGUF 변환, 모델 평가, 코퍼스 내보내기, RAG 인덱싱 | `Confirm.ask(default=config값)` | config 의존 |
