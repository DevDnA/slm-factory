...
- [7. 지식 증류와 RAG — 언제 무엇을 쓸 것인가](#7-지식-증류와-rag--언제-무엇을-쓸-것인가)
- [8. 트러블슈팅](#8-트러블슈팅)
- [관련 문서](#관련-문서)
...
| 단계 | 유형 | 설명 |
|------|:----:|------|
| 설정 파일 로드 | **필수** | `project.yaml`을 로드하고 Teacher/Student 모델을 확인합니다 |
| 문서 선택 | **필수** | 전체 또는 개별 문서를 선택합니다 |

**파이프라인 단계**

| # | 단계 | 유형 | 설명 |
|---|------|:----:|------|
| 1 | 문서 파싱 | **필수** | 선택한 문서에서 텍스트와 표를 자동 추출합니다 |
...
| 12 | RAG 인덱싱 | 선택 | corpus.parquet을 ChromaDB에 임베딩하여 적재합니다 (`rag` 설정 반영) |
...
## 7. 지식 증류와 RAG — 언제 무엇을 쓸 것인가

slm-factory는 **Teacher-Student 지식 증류**와 **RAG(검색 증강 생성)** 두 가지 기능을 제공합니다. 상황에 따라 적합한 전략이 다릅니다.

### 파이프라인 구조

```
문서 → Teacher(9B)가 QA 생성 → Student(1B) LoRA 파인튜닝 → 경량 도메인 모델
                                                              ↓
                                              RAG 서비스 (Student + 문서 검색)
```

### 문서 수에 따른 전략

| 문서 수 | 예상 QA | 지식 증류 | RAG | 추천 전략 |
|---------|---------|-----------|-----|-----------|
| 1~5건 | ~300개 이하 | ❌ 데이터 부족 — Student 과적합 | ✅ 충분 | **RAG + Teacher** |
| 10~15건 | 500~1,000개 | ⚠️ 시작 가능 | ✅ 보완 | RAG + Student (검증 필요) |
| 20건+ | 1,000개+ | ✅ 실용적 | ✅ 보완 | **RAG + Student** |
| 50건+ | 3,000개+ | ✅ 프로덕션 | ✅ 보완 | Student 단독 배포 가능 |

### 왜 문서가 적으면 증류가 안 되는가

1B~2B 모델이 도메인 지식을 내재화하려면 최소 500개 이상의 다양한 QA가 필요합니다. 문서 1건에서 100개의 QA를 만들 수 있지만, 같은 문서의 같은 내용을 반복하는 것이므로 모델은 **도메인 패턴 학습** 대신 **특정 문장 암기**를 하게 됩니다. 이는 과적합(catastrophic forgetting)으로 이어져 깨진 출력을 생성합니다.

다양한 문서에서 생성된 QA는 같은 도메인의 **다른 표현, 다른 맥락, 다른 구조**를 포함하므로 모델이 도메인 패턴을 일반화할 수 있습니다.
...
# project.yaml에서 rag.ollama_model: "" (Student 자동 사용)

# 3단계: Student 품질 검증 후 Teacher 제거 → 비용 절감
```
