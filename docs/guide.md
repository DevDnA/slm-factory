...
```bash
slm-factory train --data ./custom_qa.jsonl
```
...
slm-factory는 상황에 따라 3가지 방식으로 서비스할 수 있습니다.

#### 1. RAG 전용 — 즉시 서비스 (문서 1건부터)

```bash
slm-factory serve
```

문서를 파싱하고 벡터 인덱스를 구축한 후 바로 웹 채팅을 시작합니다. Teacher 모델(qwen3.5:9b)이 검색된 문서를 참조하여 답변합니다. 인덱스가 이미 있으면 즉시 서비스를 시작합니다.

| 소요 시간 | 모델 | 적합한 상황 |
|-----------|------|------------|
| ~30초 | Teacher (9B) | 문서 1~10건, 즉시 서비스 필요 |
...
11단계 파이프라인을 실행하여 Student 모델을 학습시키고, RAG 인덱스를 구축한 후 웹 채팅을 시작합니다. `project.yaml`의 `rag.ollama_model` 설정에 따라 Teacher 또는 Student 모델을 사용합니다.

| 소요 시간 | 모델 | 적합한 상황 |
|-----------|------|------------|
| ~30분 | Teacher 또는 Student | 문서 20건+, 경량 모델 배포 목적 |

#### 3. 병렬 실행 — RAG 즉시 + 파인튜닝 백그라운드

```bash
# 터미널 1: RAG 즉시 서비스 (30초)
slm-factory serve

# 터미널 2: 파인튜닝 진행 (30분)
slm-factory run --no-serve
```

RAG + Teacher로 즉시 서비스하면서, 동시에 Student 모델을 학습합니다. 학습이 완료되면 `project.yaml`에서 `rag.ollama_model`을 Student 모델로 변경하고 서버를 재시작합니다.

| | 터미널 1 (즉시) | 터미널 2 (백그라운드) |
|---|---|---|
| 역할 | RAG 서비스 | 파인튜닝 |
| 소요 | 30초 | 30분 |
| 모델 | Teacher (9B) | Student (1B) 학습 중 |
