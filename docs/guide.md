...
wizard는 준비 단계 후 14단계 파이프라인을 순서대로 실행합니다. 단계 번호는 [빠른 참조](quick-reference.md)의 파이프라인 다이어그램과 동일합니다.
...
| 11 | GGUF 변환 | 선택 | llama.cpp 호환 GGUF 양자화 형식으로 변환합니다 (`gguf_export.enabled` 설정 반영) |
