...
| **파인튜닝 SLM 단독** | `slf tune --no-chat` | Student(1B) | 30분~1시간 | 오프라인, 변하지 않는 지식 |
...
13단계 파이프라인을 실행하여 Student 모델을 학습시키고, RAG 인덱스를 구축한 후 웹 채팅을 시작합니다. 도메인 지식이 내재화된 경량 모델(1B)이 답변하므로 Teacher(9B) 대비 9배 빠르고 1/9 비용입니다.

#### 파인튜닝 SLM 단독 (RAG 없음)

`slf tune --no-chat`으로 파인튜닝만 수행하고, Ollama에 등록된 모델을 직접 사용합니다. RAG 없이 학습된 지식만으로 답변하므로, 변하지 않는 도메인 지식과 오프라인 환경에 적합합니다. 문서 50건 이상에서 실용적입니다.
