...
uv run slm-factory --install-completion
...
```bash
uv run slm-factory --show-completion
```
...
```bash
uv run slm-factory init my-first-project
```
...
```bash
uv run slm-factory check --config my-first-project/project.yaml
```
...
```bash
uv run slm-factory tool wizard --config my-first-project/project.yaml
```

wizard는 다음 16단계를 순서대로 실행합니다 (Step 3b는 선택적 하위 단계).

> **단계 유형 안내**
> - **필수**: 항상 실행되며 건너뛸 수 없습니다.
> - **선택**: 사용자 확인 후 건너뛸 수 있습니다. 설정의 `enabled` 값이 기본 선택에 반영됩니다.
> - **자동**: 사용자 확인 없이 자동 실행됩니다.
>
> `run` 명령의 파이프라인 단계 번호는 [빠른 참조](quick-reference.md)를 참조하십시오. wizard는 설정 로드(1)와 문서 선택(2) 단계가 추가로 포함되어 번호가 다릅니다.
...
| 6 | 품질 점수 평가 | 선택 | Teacher LLM이 각 QA를 1~5점으로 평가하여 추가 필터링합니다. 저품질 QA 재생성 옵션 포함 (`scoring.enabled` 설정 반영) |
| 7 | 데이터 증강 | 선택 | 질문을 다양한 표현으로 변형하여 학습 데이터를 늘립니다 (`augment.enabled` 설정 반영) |
| 8 | 통계 분석 | 자동 | 카테고리별 QA 분포, 길이 통계 등 분석 보고서를 생성합니다 (`analyzer.enabled` 설정 반영) |
| 9 | 데이터 변환 | **자동** | QA 데이터를 채팅 템플릿 적용 JSONL 형식으로 변환합니다 |
| 10 | LoRA 학습 | **필수** | Student 모델에 LoRA 어댑터를 적용하여 파인튜닝합니다 |
| 11 | 모델 내보내기 | **필수** | LoRA 어댑터를 기본 모델에 병합하고 Ollama Modelfile을 생성합니다 |
| 12 | 멀티턴 대화 생성 | 선택 | QA 쌍을 멀티턴 대화 형식으로 확장합니다 |
| 13 | GGUF 변환 | 선택 | llama.cpp 호환 GGUF 양자화 형식으로 변환합니다 (`gguf_export.enabled` 설정 반영) |
| 14 | 모델 평가 | 선택 | BLEU/ROUGE 메트릭으로 학습 결과를 자동 평가합니다 |
| 15 | 코퍼스 내보내기 | 선택 | QA·문서를 RAG 인덱싱용 parquet으로 내보냅니다 (`autorag_export.enabled` 설정 반영) |
| 16 | RAG 인덱싱 | 선택 | corpus.parquet을 ChromaDB에 임베딩하여 적재합니다 (`rag` 설정 반영) |
...
```bash
uv run slm-factory status --config my-first-project/project.yaml
```
...
# + GGUF 변환까지
uv run slm-factory run --until gguf_export --config project.yaml
...
```bash
# 환경 점검
uv run slm-factory check --config policy-project/project.yaml

# 전체 파이프라인 실행
uv run slm-factory run --config policy-project/project.yaml
...
```bash
# 환경 점검
uv run slm-factory check --config tech-docs/project.yaml

# 전체 파이프라인 실행
uv run slm-factory run --config tech-docs/project.yaml
...
```bash
uv run slm-factory tool review --config project.yaml
```
...
```bash
uv run slm-factory tool gguf --config project.yaml
```
...
uv run slm-factory tool wizard --resume --config project.yaml

# 또는 학습 데이터가 있으면 학습만 재실행
uv run slm-factory train --config project.yaml --data output/training_data.jsonl
```
