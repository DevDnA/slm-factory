...
# → GET  http://localhost:8000/v1/collections  ChromaDB 컬렉션 목록
```
...
| **헬스체크** | 서비스 상태 모니터링 | 내장 `/health`(기본), `/health/ready`(Ollama+ChromaDB 연결) 엔드포인트 제공. 프로덕션에서는 `/health/ready`를 로드밸런서 헬스체크에 사용 |
