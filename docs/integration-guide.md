...
slm-factory는 문서에서 엔티티·관계를 추출하여 QA 생성 프롬프트에 맥락으로 주입하는 온톨로지 기능을 제공합니다. RAG 서비스와는 별개로, **학습 데이터 품질 향상**을 위한 선택적 기능입니다. `ontology.enrich_qa: true` 설정으로 활성화합니다.

> 상세 설정은 [설정 레퍼런스](configuration.html)의 `ontology` 블록, 사용법은 [CLI 레퍼런스](cli-reference.html)의 `tool ontology`를 참고하십시오.
...
> **온톨로지 옵션**: 두 패턴 모두 `ontology.enrich_qa: true` 설정으로 QA 학습 데이터 품질을 높일 수 있습니다. 문서 간 관계가 복잡한 도메인(법률, 의료, 기술 표준)에서 효과적입니다.
...
| [설정 레퍼런스](configuration.html) | `rag`, `ontology`, `export.ollama` 설정 상세 |
