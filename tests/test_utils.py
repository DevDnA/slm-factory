...
import rag_factory.utils as utils_module
from rag_factory.utils import compute_file_hash, get_logger, run_bounded, setup_logging
...
class TestSetupLogging:
    ...
    def test_로거_반환(self):
        ...
        assert logger.name == "rag_factory"
...
class TestGetLogger:
    ...
    def test_올바른_이름의_로거_반환(self):
        """get_logger가 'rag_factory.{name}' 형식의 로거를 반환하는지 확인합니다."""
        logger = get_logger("parsers.text")
        assert isinstance(logger, logging.Logger)
        assert logger.name == "rag_factory.parsers.text"
    ...
    def test_다른_이름으로_다른_로거_반환(self):
        ...
        assert logger_a.name == "rag_factory.module_a"
        assert logger_b.name == "rag_factory.module_b"
