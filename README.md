...
> `slf rag`는 파인튜닝된 Student 모델이 Ollama에 있으면 자동으로 사용합니다. `slf rag`로 시작하고, 문서가 쌓이면 `slf tune`으로 전환하세요. 두 패턴 모두 RAG 검색 근거를 기반으로 답변하여 할루시네이션을 억제합니다.
...
> **파인튜닝은 지식 주입이 아닙니다.** 1B 모델은 문서 내용을 암기할 수 없습니다. 파인튜닝은 "주어진 문서를 읽고 도메인에 맞는 어투로 근거를 인용하며 답변하는 스타일"을 학습합니다. **지식은 항상 RAG가 담당**하고, 파인튜닝된 Student는 그 지식을 **어떻게 전달할지(톤/스타일)**를 담당합니다.

> 두 모드 모두 `--chat` 옵션으로 완료 후 웹 채팅을 자동 시작합니다. 상세 비교는 [사용 가이드](https://devdna.github.io/slm-factory/guide.html#7-파인튜닝과-rag--역할-분담)를 참고하세요.
